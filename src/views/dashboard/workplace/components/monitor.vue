<template>
  <a-card class="general-card" :title="$t('workplace.monitor.title')" :header-style="{ paddingBottom: '0' }">
    <template #extra>
      <a-button v-if="invoke" type="primary" :disabled="invokeDisable" @click="handleInvoke">
        Invoke
      </a-button>
      <a-button v-else type="secondary" :disabled="invokeDisable" @click="handleStop">
        Stop
      </a-button>
    </template>
...
import { Pointer, ALGORITHM, EVENT, DEVICESTATUS } from '@/edgelab';
