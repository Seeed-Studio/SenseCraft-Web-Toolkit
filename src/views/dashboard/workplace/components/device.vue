<template>
  <a-spin class="device-card" :loading="loading" tip="This may take a while...">
    <a-card class="general-card" :title="$t('workplace.device.title')">
      <template #extra>
        <a-tag v-if="connectStatus === DEVICESTATUS.SERIALCONNECTED" color="green">{{
      ...
      <a-space direction="vertical">
        <a-descriptions layout="horizontal" :column="1" size="large">
          <a-descriptions-item v-for="item in info" :key="item.label" :label="$t(item.label)">
            {{ item.value }}
          </a-descriptions-item>
        </a-descriptions>
        <a-button v-if="connectStatus === DEVICESTATUS.SERIALCONNECTED" type="secondary" @click="handleDisconnect">{{
          $t('workplace.device.btn.disconnect')
        }}</a-button>
        <a-button v-else type="primary" status="success" @click="handleConnect">{{ $t('workplace.device.btn.connect')
        }}</a-button>
      </a-space>
    </a-card>
  </a-spin>
...
import { PROTOCOL_LIST, DEVICESTATUS } from '@/edgelab';
