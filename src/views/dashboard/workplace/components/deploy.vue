<template>
  <div class="deployPage">
    <a-card class="general-card" :title="$t('workplace.device.card.burnfirmware')">
      <template #extra>
...
import { DEVICESTATUS } from '@/edgelab';
import Serial from '@/edgelab/serial';
...
const bootloaderFile: Ref<File | null> =
  ref(null);
const partiontableFile: Ref<File | null> =
  ref(null);
...
const haveBurnFile = computed(() => {
  return bootloaderFile.value || partiontableFile.value || appFile.value || aiModelFile.value;
});
    ...
    if (file) {
      ...
      reader.onload = (ev: ProgressEvent<FileReader>) => {
        ...
        if (type === FILE_TYPE.MODEL_FILE) {
          ...
          if (data.indexOf('abort() was called at PC 0x%08') > -1 && size < 100 * 1024) {
            if (bootloaderFile.value) {
          ...
          } else if (data.indexOf('factory') > -1 && data.indexOf('nvs') > -1 && size < 10 * 1024) {
            if (partiontableFile.value) {
              Message.error('The same file exists');
              wrongFile = true;
            } else {
          ...
          } else if (data.indexOf('esp_hw_support') > -1 && size < 4 * 1024 * 1024) {
            if (appFile.value) {
              Message.error('The same file exists');
              wrongFile = true;
            } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (file) {
      if (type === FILE_TYPE.MODEL_FILE) {
        ...
        if (file.name === bootloaderFile.value?.name && file.size === bootloaderFile.value?.size) {
          bootloaderFile.value = null;
        } else if (file.name === partiontableFile.value?.name && file.size === partiontableFile.value?.size) {
          partiontableFile.value = null;
        } else if (file.name === appFile.value?.name && file.size === appFile.value?.size) {
          appFile.value = null;
        }
      }
      resolve(true);
