<template>
  <a-card class="general-card" :title="$t('workplace.config.title')">
    <a-form :model="{}" layout="vertical">
      <a-form-item :label="$t('workplace.config.model.label')" required>
        <a-select v-model="model">
          <a-option v-for="item in modelList" :key="item.value" :value="item.value" :label="$t(item.label)" />
        </a-select>
      </a-form-item>
      <a-form-item :label="$t('workplace.config.algorithm.label')" required>
        <a-select v-model="algorithm">
          <a-option v-for="item in algorithmList" :key="item.value" :value="item.value" :label="$t(item.label)" />
        </a-select>
      </a-form-item>
      <div>
        <a-space>
          <a-form-item :label="$t('workplace.config.confidence.label')" required>
            <a-input-number v-model="confidence" mode="button" style="min-width: 120px" :min="0" :max="100" />
          </a-form-item>
          <a-form-item :label="$t('workplace.config.nms.iou.label')" required>
            <a-input-number v-model="iou" mode="button" style="min-width: 120px" :min="0" :max="100" />
          </a-form-item>
        </a-space>
        <div v-if="algorithm === ALGORITHM.POINTER_METER">
          <a-space>
            <a-form-item :label="$t('workplace.config.pointer.start')" required>
              <a-input-group style="min-width: 180px">
                <a-button :type="isStart ? 'secondary' : 'primary'" status="success" style="min-width: 30px"
                  @click="handelPosition('start')"><template #icon><icon-location /></template></a-button>
            ...
            <a-form-item :label="$t('workplace.config.pointer.end')" required>
              <a-input-group style="min-width: 180px">
                <a-button :type="isEnd ? 'secondary' : 'primary'" status="success" style="min-width: 30px"
                  @click="handelPosition('end')"><template #icon><icon-location /></template></a-button>
          ...
          <a-space>
            <a-form-item :label="$t('workplace.config.pointer.center')" required>
              <a-input-group style="min-width: 180px">
                <a-button :type="isCenter ? 'secondary' : 'primary'" status="success" style="min-width: 30px"
                  @click="handelPosition('center')"><template #icon><icon-location /></template></a-button>
            ...
            <a-form-item :label="$t('workplace.config.pointer.range')" required>
              <a-input-group style="min-width: 180px">
                <a-button type="primary" status="warning" style="min-width: 30px"><template
                    #icon><icon-record /></template></a-button>
    ...
    <a-button type="primary" :loading="saveLoading" :disabled="connectStatus!==DEVICESTATUS.SERIALCONNECTED" @click="handelSave">{{
      $t('workplace.config.btn.save') }}</a-button>
    <span :style="{ width: '15px', display: 'inline-block' }"></span>
    <a-button type="secondary" :loading="saveLoading" :disabled="connectStatus!==DEVICESTATUS.SERIALCONNECTED" @click="handelRefresh">{{
...
import { ALGORITHM, EVENT, DEVICESTATUS } from '@/edgelab';
...
watch(
  () => device?.getEvent(),
  (val) => {
    if (val === EVENT.CONFIG_POINTER_DONE && device) {
      if (isStart.value) {
